...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        """
        # Completed add code as required
        int_idx = int(cd_idx)
        try:
            for row in table:
                if row.cd_id == int_idx:
                    return row
        except:
            raise Exception ('Incorrect ID selected.')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        # Completed add code as required
        position1, title1, length1 = track_info
        try:
        ...
        track = DC.Track(position1, title1, length1)
        cd.add_track(track)